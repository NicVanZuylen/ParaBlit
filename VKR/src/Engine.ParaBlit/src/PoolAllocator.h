...
namespace PB
{
	...
	public:
		...
		void Init(Renderer* renderer, EMemoryType memoryType, uint32_t poolSize, uint32_t minAlignmentBytes, uint64_t poolMemoryLimit, const CLib::Vector<uint32_t>& segments);
	...
	private:
		...
		uint64_t m_totalAllocatedMemory = 0;
		uint64_t m_memoryHead = 0;
		uint64_t m_poolMemoryLimit = ~uint64_t(0);
		std::mutex m_mutex;
		std::map<void*, PoolInfo> m_pools;
