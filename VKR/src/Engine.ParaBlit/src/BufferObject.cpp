...
namespace PB
{
    ...
    VkDeviceAddress BufferObject::GetDeviceAddress() const
    {
		PB_ASSERT_MSG((m_usage & (PB::EBufferUsage::MEMORY_ADDRESS_ACCESS | PB::EBufferUsage::SHADER_BINDING_TABLE)) > 0
			, "Buffer object must have EBufferUsage::MEMORY_ADDRESS_ACCESS or EBufferUsage::SHADER_BINDING_TABLE usage flags specified in the desc.");

		VkBufferDeviceAddressInfo addressInfo{ VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO, nullptr };
		addressInfo.buffer = m_handle;

		return vkGetBufferDeviceAddress(m_renderer->GetDevice()->GetHandle(), &addressInfo);
    }
	...
	{
		void* data = nullptr;
		if (m_memoryType == EMemoryType::HOST_VISIBLE || m_memoryType == EMemoryType::HOST_VISIBLE_AND_DEVICE_LOCAL)
		{
	...
	void BufferObject::EndPopulate(BufferCopyRegion* regions, u32 regionCount)
	{
		...
		Renderer::t_threadResourceInitializationCommandContext.Pause();
	...
	inline void BufferObject::CreateVkBuffer(const BufferObjectDesc& desc)
	{
		...
		if (desc.m_name)
		{
			m_renderer->RegisterObjectName(desc.m_name, uint64_t(m_handle), VK_OBJECT_TYPE_BUFFER);
		}
#endif
		...
		if (!(desc.m_options & PB::EBufferOptions::POOL_PLACED)) // Skip memory allocation & binding for placed buffers, as they will later be placed on a memory heap.
		{
			...
			{
				if (desc.m_options & EBufferOptions::DEVICE_MEMORY)
				{
					m_memoryType = EMemoryType::HOST_VISIBLE_AND_DEVICE_LOCAL;
				}
				else
				{
