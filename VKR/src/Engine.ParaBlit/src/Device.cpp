...
namespace PB 
{
	...
	void Device::Init(VkInstance instance, Renderer* renderer, const DeviceDesc& desc)
	{
		...
		EnumDevice();
		CreateLogicalDevice(m_desc.enableSwapchainExtension);
...

		{
			...
			m_deviceBufferAllocator.Init(m_renderer, EMemoryType::DEVICE_LOCAL, BufferPoolSize, BufferPoolMinAlignment, 0, bufferAllocatorSegments);
			m_hostBufferAllocator.Init(m_renderer, EMemoryType::HOST_VISIBLE, BufferPoolSize, BufferPoolMinAlignment, 0, bufferAllocatorSegments);
		}

		// Host Visible Device Local memory usually has a much smaller available memory pool.
		{
			const uint32_t PoolSizeMB = m_limitations.m_hostVisibleDeviceLocalPoolSize / (1024 * 1024);
			const uint32_t BufferPoolSize = PoolSizeMB * 128;
			static constexpr uint32_t BufferPoolMinAlignment = 256; // Common alignment requirement for Vulkan buffers.
			CLib::Vector<uint32_t> bufferAllocatorSegments =
			{
				0,
			...
			m_hostDeviceBufferAllocator.Init(m_renderer, EMemoryType::HOST_VISIBLE_AND_DEVICE_LOCAL, BufferPoolSize, 256, PoolSizeMB * 1024 * 1024, bufferAllocatorSegments);
		}

		{
			...
			m_deviceTextureAllocator.Init(m_renderer, EMemoryType::DEVICE_LOCAL, TexturePoolSize, TexturePoolMinAlignment, 0, texAllocatorSegments);
			m_hostTextureAllocator.Init(m_renderer, EMemoryType::HOST_VISIBLE, TexturePoolSize, TexturePoolMinAlignment, 0, texAllocatorSegments);
		}
	}
	...
	inline VkMemoryPropertyFlags GetVkMemPropertyFlags(const EMemoryType& memType)
	{
		...
		case EMemoryType::HOST_VISIBLE:
			...
			break;
		case EMemoryType::HOST_VISIBLE_AND_DEVICE_LOCAL:
	...
	void Device::EnumDevice()
	{
		...
		for (u32 i = 0; i < deviceCount; ++i)
		{
			...
			{
				m_physDeviceAccelerationStructureFeatures.pNext = &m_physDeviceRayTracingPipelineFeatures;
			}
			...
			m_physDeviceVulkan11Properties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_1_PROPERTIES;
			...
			m_physDeviceAccelerationStructureProperties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_PROPERTIES_KHR;
			m_physDeviceRayTracingPipelineProperties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_PROPERTIES_KHR;

			deviceProperties.pNext = &m_physDeviceVulkan11Properties;
			m_physDeviceVulkan11Properties.pNext = &m_physDeviceDescIndexingProps;
			m_physDeviceDescIndexingProps.pNext = &m_physDeviceMeshShaderProperties;
			m_physDeviceMeshShaderProperties.pNext = &m_physDeviceAccelerationStructureProperties;

			if (m_desc.enableRaytracingCapabilities)
			{
				m_physDeviceAccelerationStructureProperties.pNext = &m_physDeviceRayTracingPipelineProperties;
			}
	...
	void Device::EnableExtensions(ExtensionManager& extManager, bool enableSwapchainExtension)
	{
		...
		if (m_desc.enableRaytracingCapabilities)
		{
			m_limitations.m_supportRaytracing = extManager.EnableExtension(VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME)
				&& extManager.EnableExtension(VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME)
				&& extManager.EnableExtension(VK_KHR_RAY_TRACING_PIPELINE_EXTENSION_NAME);
		}
	...
	void Device::SetFeatureEnables()
	{
		...
		m_physDeviceRayTracingPipelineFeatures.rayTracingPipelineShaderGroupHandleCaptureReplayMixed = VK_FALSE;
		m_physDeviceRayTracingPipelineFeatures.rayTracingPipelineTraceRaysIndirect = VK_TRUE;
		m_physDeviceRayTracingPipelineFeatures.rayTraversalPrimitiveCulling = VK_TRUE;
	}

	void Device::QueryDeviceLimitations()
	{
		// The heap size for memory which is both HOST_VISIBLE and DEVICE_LOCAL is usually very small, so it is worth tracking it to ensure we don't run out of this memory.
		{
			u32 memoryTypeIndex = FindMemoryTypeIndex(~u32(0), EMemoryType::HOST_VISIBLE_AND_DEVICE_LOCAL);
			VkMemoryType memType = m_memoryProperties.memoryTypes[memoryTypeIndex];

			if (memType.heapIndex != ~uint32_t(0))
			{
				m_limitations.m_hostVisibleDeviceLocalPoolSize = m_memoryProperties.memoryHeaps[memType.heapIndex].size;
			}
		}

		m_limitations.m_subgroupSize = m_physDeviceVulkan11Properties.subgroupSize;
	}
