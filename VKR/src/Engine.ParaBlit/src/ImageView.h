...
namespace PB
{
	...
	public:
		...
		VkImageView GetVkImageView(const TextureViewDesc& desc);
		...
		VkSampler GetVkSampler(const SamplerDesc& desc);
	...
	private:
		...
		TextureViewData& GetTextureViewData(const TextureViewDesc& desc);

		SamplerData& GetSamplerData(const SamplerDesc& desc);
		...
		std::mutex m_uboCacheMutex;
		std::mutex m_ssboCacheMutex;
		std::mutex m_samplerCacheMutex;
	};
}
