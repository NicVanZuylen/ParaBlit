...
#define PB_DEFINE_ENUM_FIELD(name, enumType, numType)																					\
	...
	enum class EBufferUsage : u32
	{
		...
		INDEX							=	1 << 5,
		INDIRECT_PARAMS					=	1 << 6,
		ACCELERATION_STRUCTURE_STORAGE	=	1 << 7,
		MEMORY_ADDRESS_ACCESS			=	1 << 8,
		SHADER_BINDING_TABLE			=	1 << 9
	};
	...
	enum ERayTracingShaderStage : u32
	{
		RAY_GEN,
		MISS,
		CLOSEST_HIT,
	...
	struct SubresourceRange
	{
		...
		static SubresourceRange AllMipLevels()
		{
			SubresourceRange ret;
			ret.m_baseMip = 0;
			ret.m_mipCount = ~u16(0);
		...
		static SubresourceRange AllArrayLayers()
		{
			SubresourceRange ret;
			ret.m_baseMip = 0;
			ret.m_mipCount = 1;
		...
		bool IsAllMipLevels() const { return m_mipCount == 0xFFFF; }
		bool IsAllArrayLayers() const { return m_arrayCount == 0xFFFF; }
	...
	{
		u64 m_hostVisibleDeviceLocalPoolSize = 0;
		u32 m_subgroupSize = 0;
