...
namespace PB
{
	...
	{
		static constexpr u32 MaxShadersPerStage = 8;

		ShaderModule rayGenShaderModule = 0;
		ShaderModule missShaderModules[MaxShadersPerStage]{};
		ShaderModule closestHitShaderModules[MaxShadersPerStage]{};
		u32 maxPipelineRecursionCount = 1;

		bool operator == (const RayTracingPipelineDesc& other) const;
	};
	static_assert(sizeof(RayTracingPipelineDesc) % 16 == 0);
	...
	public:
		...
		Param:
			const GraphicsPipelineDesc& desc: The pipeline descriptor object containing parameters that are used to create the pipeline and identify it in the cache.
		*/
		...
		Param:
			const ComputePipelineDesc& desc: The pipeline descriptor object containing parameters that are used to create the pipeline and identify it in the cache.
		*/
		PARABLIT_INTERFACE Pipeline GetPipeline(const ComputePipelineDesc& desc) = 0;

		/*
		Description: Get or create a raytracing pipeline using the provided pipeline descriptor object.
		Param:
			const RayTracingPipelineDesc& desc: The pipeline descriptor object containing parameters that are used to create the pipeline and identify it in the cache.
		*/
		PARABLIT_INTERFACE Pipeline GetPipeline(const RayTracingPipelineDesc& desc) = 0;
	};
}