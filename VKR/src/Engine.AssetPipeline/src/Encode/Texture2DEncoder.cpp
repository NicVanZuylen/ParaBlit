...
namespace AssetPipeline
{
	...
	void Texture2DEncoder::EncodeEnvironmentMap(const AssetStatus& asset, Ctrl::TObjectPtr<TextureAsset> assetData, bool& assetMetaDataModified)
	{
		...
		// Skybox asset:
		{
			...
			PB::u32 compressedFaceDataSize = FastBlockCompression::CalcBC6TextureSizeBytes(SkyboxDimensions, SkyboxDimensions);
			...
			for (uint32_t face = 0; face < 6; ++face)
			{
				...
				if (compress)
				{
					...
					//constexpr int RowStride = SkyboxDimensions * 4;
					//constexpr int ColumnStride = 4;
					//CompressBlockBC6((unsigned short*)mappedData, RowStride, ColumnStride, cmpBlock, bc6Options);
			...
			for (uint32_t layer = 0; layer < 6; ++layer)
			{
				for (uint32_t mip = 0; mip < ConvolutionMipmapCount; ++mip)
				{
					...
					if (compress)
					{
						...
						readbackSize = FastBlockCompression::CalcBC6TextureSizeBytes(widthPixels, heightPixels);
					}
			...
			for (uint32_t layer = 0; layer < 6; ++layer)
			{
				...
				for (uint32_t mip = 0; mip < ConvolutionMipmapCount; ++mip)
				{
					...
					if (compress)
					{
						...
						FastBlockCompression::CompressBC6(mappedData, &outData[offset], widthPixels, heightPixels);
