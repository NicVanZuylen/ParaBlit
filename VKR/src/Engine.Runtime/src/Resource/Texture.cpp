...
namespace Eng
{
	...
	Texture::Texture(PB::IRenderer* renderer, CLib::Allocator* allocator, const char* filePath, bool srgb, bool convertToCube, bool loadFromDatabase)
	{
		...
		if (loadFromDatabase)
		{
			...
			if constexpr (TEXTURE_USE_TEXTURE_LABELS)
			{
				bufferName = filePath;
			}
			...
			texDesc.m_name = bool(TEXTURE_USE_TEXTURE_LABELS) ? bufferName.c_str() : nullptr;
		...
		else
		{
			...
			if (data)
			{
				...
				textureDesc.m_name = bool(TEXTURE_USE_TEXTURE_LABELS) ? bufferName.c_str() : nullptr;
	...
	void Texture::Load2D(AssetEncoder::AssetID assetID, AssetEncoder::AssetBinaryDatabaseReader* reader)
	{
		...
		if constexpr (TEXTURE_USE_TEXTURE_LABELS)
		{
			bufferName = s_textureDatabaseLoader.GetAssetName(assetID);
		}
	...
	void Texture::Load2DArray(AssetEncoder::AssetID* assetIDs, uint32_t layerCount, AssetEncoder::AssetBinaryDatabaseReader* reader)
	{
		CLib::Vector<PB::ITexture*> layers;

		assert(reader && reader->HasOpenFile());
		assert(assetIDs != nullptr);
		assert(layerCount > 0);

		CLib::Vector<PB::TextureDataDesc, 8, 8> textureLayersData(layerCount);

		AssetPipeline::TextureMetadata firstLayer;
		PB::TextureDataDesc* lastDesc = nullptr;
		for (uint32_t layer = 0; layer < layerCount; ++layer)
		{
			const auto& textureMeta = reader->GetAssetInfo(assetIDs[layer]);
			AssetPipeline::TextureMetadata textureData;
			s_textureDatabaseLoader.GetAssetUserData(textureMeta, &textureData);

			if (layer == 0)
			{
				firstLayer = textureData;
			}
			else
			{
				assert(textureData.m_format == firstLayer.m_format);
				assert(textureData.m_width == firstLayer.m_width);
				assert(textureData.m_height == firstLayer.m_height);
			}

			PB::TextureDataDesc& texDataDesc = textureLayersData.PushBack();
			texDataDesc.m_data = malloc(textureMeta.m_binarySize);
			texDataDesc.m_size = textureMeta.m_binarySize;
			texDataDesc.m_mipLevel = 0;
			texDataDesc.m_arrayLayer = layer;
			texDataDesc.m_next = nullptr;

			if (lastDesc)
			{
				lastDesc->m_next = &texDataDesc;
			}
			lastDesc = &texDataDesc;

			s_textureDatabaseLoader.GetAssetBinary(assetIDs[layer], texDataDesc.m_data);

			assert(textureData.m_isHdr == false);

			printf("Texture: Successfully loaded asset [%u] (%u bytes) as a texture array layer.\n", uint32_t(assetIDs[layer]), uint32_t(textureMeta.m_binarySize));
		}

		std::string bufferName;
		if constexpr (TEXTURE_USE_TEXTURE_LABELS)
		{
			bufferName = s_textureDatabaseLoader.GetAssetName(assetIDs[0]);
			bufferName += "_array";
		}
		...
		texDesc.m_name = bool(TEXTURE_USE_TEXTURE_LABELS) ? bufferName.c_str() : nullptr;
		texDesc.m_dimension = PB::ETextureDimension::DIMENSION_2D;
		texDesc.m_format = firstLayer.m_format;
		texDesc.m_usageStates = PB::ETextureState::SAMPLED;
		texDesc.m_initOptions = PB::ETextureInitOptions::PB_TEXTURE_INIT_USE_DATA;
		texDesc.m_data = textureLayersData.Data();
		texDesc.m_width = firstLayer.m_width;
		texDesc.m_height = firstLayer.m_height;
		texDesc.m_mipCount = 1;
		texDesc.m_arraySize = layerCount;

		m_texture = m_renderer->AllocateTexture(texDesc);
		m_ownsTexture = true;
		m_isCompressed = PB::u16(texDesc.m_format) >= PB::BlockCompressedStart && PB::u16(texDesc.m_format) <= PB::BlockCompressedEnd;

		for (auto& data : textureLayersData)
		{
			free(data.m_data);
		}
	}
	...
	void Texture::ConvertToCube(bool isHdr)
	{
		...
		PB::Pipeline cubeGenPipeline = 0;
		{
			...
			cubeGenPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::VERTEX] = Eng::Shader(m_renderer, "Shaders/GLSL/vs_cubegen", 0, m_allocator, true).GetModule();
			cubeGenPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::FRAGMENT] = Eng::Shader(m_renderer, "Shaders/GLSL/fs_cubegen", 0, m_allocator, true).GetModule();
