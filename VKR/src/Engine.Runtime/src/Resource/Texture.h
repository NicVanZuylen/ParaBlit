...
namespace Eng
{
	...
	public:
		...
		*/
		...
		/*
		Description: Load and initialize a texture array from a list of texture asset IDs. Mip levels are ignored.
		Param:
			AssetEncoder::AssetID: The asset IDs used for loading the texture array layers.
			uint32_t layerCount: Amount of texture assets to load and add as array layers to this texture.
			AssetEncoder::AssetBinaryDatabaseReader* reader: Database reader the texture will be loaded using.
		*/
		void Load2DArray(AssetEncoder::AssetID* assetIDs, uint32_t layerCount, AssetEncoder::AssetBinaryDatabaseReader* reader = &s_textureDatabaseLoader);
