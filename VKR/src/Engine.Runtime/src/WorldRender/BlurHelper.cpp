...
namespace Eng
{
	void BlurHelper::Init(PB::IRenderer* renderer, PB::u32 kernelSize)
	{
		...
		EBlurKernelSize blurPermutation = BlurKernelSizeToEnum(kernelSize);

		AssetEncoder::ShaderPermutationTable<EAxisPermutation> permTable{};
		permTable.SetPermutation(EAxisPermutation::PERMUTATION_HORIZONTAL, 0);
		permTable.SetPermutation(EAxisPermutation::PERMUTATION_KERNEL_SIZE, uint8_t(blurPermutation));
		m_verticalPassModule = Shader(m_renderer, "Shaders/GLSL/cs_compute_gaussian_blur", permTable.GetKey(), nullptr, true).GetModule();
		permTable.SetPermutation(EAxisPermutation::PERMUTATION_HORIZONTAL, 1);
		m_horizontalPassModule = Shader(m_renderer, "Shaders/GLSL/cs_compute_gaussian_blur", permTable.GetKey(), nullptr, true).GetModule();

		PB::BufferObjectDesc constantsDesc{};
		constantsDesc.m_bufferSize = sizeof(BlurConstants) + (sizeof(PB::Float4) * m_kernelSize);
		...
		float kernelSampleCount = (float)m_kernelSize;
		...
		for (PB::u32 i = 0; i < m_kernelSize; ++i)
		{
