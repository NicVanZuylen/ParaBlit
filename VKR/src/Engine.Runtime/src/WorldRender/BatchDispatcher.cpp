...
namespace Eng
{
	...
	BatchDispatcher::BatchDispatcher(PB::IRenderer* renderer, CLib::Allocator* allocator)
	{
		...
		cullPipelineDesc.m_computeModule = Eng::Shader(m_renderer, "Shaders/GLSL/cs_drawbatch_cull", 0, allocator, true).GetModule();
		...
		{
			cullPipelineDesc.m_computeModule = Eng::Shader(m_renderer, "Shaders/GLSL/cs_drawbatch_cull_tasks", 0, allocator, true).GetModule();
	...
	void BatchDispatcher::DrawBatches(PB::ICommandContext* cmdContext, PB::UniformBufferView viewConstantsView, PB::Pipeline overridePipeline, bool drawExperimental)
	{
		...
		for (auto& pipelineBatches : m_batches)
		{
			...
			for (auto& batchState : pipelineBatches.second)
			{
				...
				{
					batchState.batch->LocalDrawAllMeshShader(cmdContext, batchState.batchBindings, viewConstantsView);
				}
			}
		}
