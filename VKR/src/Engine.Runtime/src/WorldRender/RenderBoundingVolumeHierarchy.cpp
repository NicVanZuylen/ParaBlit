...
namespace Eng
{
	...
	void RenderBoundingVolumeHierarchy::BuildDrawBatch(PB::Pipeline pipeline, BuildNode* node, BatchObjects& objects)
	{
		...
		for (auto& node : objects)
		{
			...
			pipelineBatch.m_batch->AddInstance(obj->m_meshID, (const float*)obj->m_transform, node->m_bounds, obj->m_material->GetTextureIDs(), obj->m_material->GetTextureCount(), m_renderer->GetSampler(batchTextureSamplerDesc));
		}
	...
	void RenderBoundingVolumeHierarchy::RebuildDrawBatchesForSubtree(BuildNode* subtree, BuildNode* oldSubtree, InputObjects& objectData)
	{
		...
		for (const BoundingVolumeHierarchy::ObjectData*& obj : objectData)
		{
			...
			// TODO: This can be set to an actual pipeline handle. Use when multiple pipeline support is added.
			PB::Pipeline pipeline = 0;
	...
	void RenderBoundingVolumeHierarchy::GetBatchCandidates(BatchMap& batchMap, BuildNode* node)
	{
		...
		if (node->m_isObject)
		{
			...
			PB::Pipeline pipeline = 0; // TODO: This can be set to an actual pipeline handle. Use when multiple pipeline support is added.
