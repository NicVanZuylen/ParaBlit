...
namespace Eng
{
	...
	void ShadowAccumPass::OnPassBegin(const RenderGraphInfo& info, PB::RenderTargetView* renderTargetViews, PB::ITexture** transientTextures)
	{
		auto RecordPassA = [&]()
		{
			...
			accumPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::VERTEX] = Shader(m_renderer, "Shaders/GLSL/vs_screenQuad", 0, m_allocator, true).GetModule();
			accumPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::FRAGMENT] = Shader(m_renderer, "Shaders/GLSL/fs_shadow_accum", 0, m_allocator, true).GetModule();
