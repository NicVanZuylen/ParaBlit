...
namespace Eng
{
	...
	struct AttachmentDesc
	{
		...
		PB::ITexture* m_externalTexture = nullptr;
		uint32_t m_width = 0;
		uint32_t m_height = 0;
	...
	public:
		...
		struct TransientTextureMeta
		{
			...
			uint32_t m_width = 0;
			uint32_t m_height = 0;
			PB::TextureStateFlags m_usage = 0;
			PB::ETextureFormat m_format = PB::ETextureFormat::UNKNOWN;
		...
		const TransientTextureMeta* GetTextureData(const char* name);

		void RegisterUserTexture(const char* rgName, PB::ITexture* texture);

		RenderGraph* Build(bool useDynamicRenderPasses);

		void Build(RenderGraph* graph, bool useDynamicRenderPasses);
	...
	public:
		...
		void Execute(PB::ICommandContext* commandContext = nullptr);
