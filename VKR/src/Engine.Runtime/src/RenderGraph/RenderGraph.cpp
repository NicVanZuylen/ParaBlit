...
namespace Eng
{
	...
	{
		m_name = desc.m_name;
		m_externalTexture = desc.m_externalTexture;
	...
	const RenderGraphBuilder::TransientTextureMeta* RenderGraphBuilder::GetTextureData(const char* name)
	{
	...
	void RenderGraphBuilder::RegisterUserTexture(const char* rgName, PB::ITexture* texture)
	{
		auto it = m_texUsageDatas.find(rgName);
		RG_ASSERT(it != m_texUsageDatas.end() && "No RenderGraph resource exists with the provided name.");

		auto& data = it->second;
		RG_ASSERT(data.m_meta.m_externalTexture == nullptr || data.m_meta.m_externalTexture == texture && "The specified RenderGraph resource already has a user texture provided.");

		data.m_meta.m_externalTexture = texture;
	...
	RenderGraph* RenderGraphBuilder::Build(bool useDynamicRenderPasses)
	{
		...
		Build(graph, useDynamicRenderPasses);

		return graph;
	}

	void RenderGraphBuilder::Build(RenderGraph* graph, bool useDynamicRenderPasses)
	{
		RG_ASSERT(graph);
	...
	void RenderGraphBuilder::UpdateTextureUsageData(const TransientTextureMeta& meta, PB::ETextureState initialUsage, uint32_t timePoint)
	{
		...
		if (it != m_texUsageDatas.end())
		{
			...
			RG_ASSERT(attach.m_texture == meta.m_externalTexture && "New external texture provided with duplicate name.");
		...
		else
		{
			...
			newAttach.m_texture = meta.m_externalTexture;
	...
	inline uint32_t GetFormatBytesPerPixel(PB::ETextureFormat format)
	{
		...
		case PB::ETextureFormat::B8G8R8A8_SRGB:
			return 4;
		case PB::ETextureFormat::A2R10G10B10_UNORM:
			return 4;
	...
	void RenderGraph::Execute(PB::ICommandContext* commandContext)
	{
		PB::ICommandContext* cmdContext = commandContext ? commandContext : PB::CreateCommandContext(m_passInfo.m_renderer);
		...
		m_passInfo.m_commandContext = cmdContext;
		...
		if (commandContext == nullptr)
		{
			...
			PB::DestroyCommandContext(cmdContext);
		}
	}
