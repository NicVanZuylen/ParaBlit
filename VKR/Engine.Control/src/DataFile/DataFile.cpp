...
namespace Ctrl
{
	...
	DataNode::DataNode(pugi::xml_node& node)
	{
	...
	void DataNode::InitializeWithNode(pugi::xml_node& node)
	{
		...
		for (auto& a : m_attributes)
		{
			...
			switch (attribute.m_type)
			{
				...
				case EAttributeType::GUID:
				{
					...
					const char* guids = GetGUID(a.first.c_str(), guidCount);
				...
				{
					DataNode** n = reinterpret_cast<DataNode**>(attribute.m_data.Data());
	...
	int DataNode::GetInteger(const char* name) const
	{
		uint32_t count = 0;
		int val = 0;
		val = *GetInteger(name, count);
		return val;
	}
	...
	float DataNode::GetFloat(const char* name) const
	{
		uint32_t count = 0;
		float val = 0;
		val = *GetFloat(name, count);
		return val;
	}
	...
	double DataNode::GetDouble(const char* name) const
	{
		uint32_t count = 0;
		double val = 0;
		val = *GetDouble(name, count);
		return val;
	}
	...
	bool DataNode::GetBool(const char* name) const
	{
		uint32_t count = 0;
		bool val = 0;
		val = *GetBool(name, count);
		return val;
	}
	...
	const char* DataNode::GetString(const char* name) const
	{
	...
	const char* DataNode::GetGUID(const char* name, uint32_t& outCount) const
	{
		auto it = m_attributes.find(name);
		if (it != m_attributes.end())
		{
			...
			outCount = attrib.m_data.Count() / GUIDLength;
			return reinterpret_cast<const char*>(it->second.m_data.Data());
		}

		return nullptr;
	}

	const char* DataNode::GetGUID(const char* name) const
	{
		uint32_t count = 0;
	...
	void DataNode::SetGUID(const char* name, const char** value, uint32_t count)
	{
		std::string nameStr = name;
		...
		if (it != m_attributes.end())
		{
			Attribute& attrib = it->second;
			assert(attrib.m_type == EAttributeType::GUID);
			attrib.m_data.SetCount(count * GUIDLength);

			for (uint32_t i = 0; i < count; ++i)
			{
				uint32_t pos = i * GUIDLength;
				std::memcpy(attrib.m_data.Data() + pos, value[i], GUIDLength);
			}
		}

		Attribute& newAttribute = m_attributes.emplace(std::pair<std::string, Attribute>(nameStr, {})).first->second;
		newAttribute.m_type = EAttributeType::GUID;
		newAttribute.m_data.SetCount(count * GUIDLength);

		for (uint32_t i = 0; i < count; ++i)
		{
			uint32_t pos = i * GUIDLength;
			std::memcpy(newAttribute.m_data.Data() + pos, value[i], GUIDLength);
		}
	}

	void DataNode::SetGUID(const char* name, const char* value)
	{
		SetGUID(name, &value);
	}

	void DataNode::RemoveAttributeOrNode(const char* name)
	{
		auto it = m_attributes.find(name);
		if (it != m_attributes.end())
		{
			FreeAttributeData(it->second);
	...
	IDataNode* DataNode::GetOrAddDataNode(const char* name, const char* type)
	{
		...
		nodePtr = s_dataAllocator.Alloc<DataNode>();
	...
	void DataNode::ReadChildren(pugi::xml_node& node)
	{
		...
		if (node.attributes().empty()) // Node is a typeless data container.
		{
			Attribute& newAttribute = m_attributes.emplace(std::pair<std::string, Attribute>(nameStr, {})).first->second;
			newAttribute.m_type = EAttributeType::DATA_NODE;
			newAttribute.m_data.SetCount(sizeof(DataNode*));

			*reinterpret_cast<DataNode**>(newAttribute.m_data.Data()) = s_dataAllocator.Alloc<DataNode>(node);
			return;
		}
		...
		if (validType && validValue)
		{
			...
			{
				CLib::Vector<const char*> guidArr;
				std::string valueStr = valueAttrib.value();
				size_t pos = 0;
				while (pos != std::string::npos)
				{
	...
	void DataNode::FreeAttributeData(Attribute& attribute)
	{
		switch (attribute.m_type)
		{
			case EAttributeType::DATA_NODE:
	...
	void DataNode::StringifyGUIDArray(std::stringstream& outStr, const char* guidArray, uint32_t count)
	{
		for (uint32_t i = 0; i < count; ++i)
		{
			if (i != 0)
			...
			outStr.write(&guidArray[i * GUIDLength], GUIDLength);
	...
	void DataFile::Open(const char* fileName, EOpenMode openMode)
	{
	...
	IDataFile* IDataFile::Create(const char* fileName, EOpenMode openMode)
	{
