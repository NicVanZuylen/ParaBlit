...
namespace Eng
{
	...
	void DeferredLightingPass::OnPassBegin(const RenderGraphInfo& info, PB::RenderTargetView* renderTargetViews, PB::ITexture** transientTextures)
	{
		...
		auto RecordPass = [&]()
		{
			...
			if (m_skyboxTexture != nullptr)
			{
				...
				skyboxPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::VERTEX] = Eng::Shader(m_renderer, "Shaders/GLSL/vs_skybox", m_allocator, true).GetModule();
				skyboxPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::FRAGMENT] = Eng::Shader(m_renderer, "Shaders/GLSL/fs_skybox", m_allocator, true).GetModule();
	...
	void DeferredLightingPass::GenSpecBDRFLut(PB::ICommandContext* cmdContext)
	{
		...
		PB::Pipeline genBDRFPipeline = 0;
		{
			...
			genBDRFPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::VERTEX] = Eng::Shader(m_renderer, "Shaders/GLSL/vs_screenQuad", m_allocator, true).GetModule();
			genBDRFPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::FRAGMENT] = Eng::Shader(m_renderer, "Shaders/GLSL/fs_gen_spec_bdrf_lut", m_allocator, true).GetModule();
