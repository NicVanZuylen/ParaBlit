...
namespace Eng
{
	...
	void AmbientOcclusionPass::OnPassBegin(const RenderGraphInfo& info, PB::RenderTargetView* renderTargetViews, PB::ITexture** transientTextures)
	{
		auto RecordPassA = [&]()
		{
			...
			ssaoPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::VERTEX] = Eng::Shader(m_renderer, "Shaders/GLSL/vs_screenQuad", m_allocator, true).GetModule();
			ssaoPipelineDesc.m_shaderModules[PB::EGraphicsShaderStage::FRAGMENT] = Eng::Shader(m_renderer, "Shaders/GLSL/fs_ssao", m_allocator, true).GetModule();
