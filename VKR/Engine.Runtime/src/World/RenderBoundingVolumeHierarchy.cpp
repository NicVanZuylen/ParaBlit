...
namespace Eng
{
	...
	void RenderBoundingVolumeHierarchy::BuildBottomUp(CLib::Vector<ObjectData>& objects)
	{
		...
		BatchMap batchMap;
		BuildDrawBatches(m_root, batchMap);
	}
	...
	void RenderBoundingVolumeHierarchy::BuildDrawBatches(BuildNode* root, BatchMap& batchMap)
	{
		...
		// This assertion is a sanity check and should never fail.
		// For failure to occur the node would need to be an object with more than DrawBatch::MaxObjects (256) child objects.
		...
		for (auto pair : batchMap)
		{
			...
			else
			{
				for (auto* child : root->m_children)
				{
					BuildDrawBatches(child, batchMap);
				}
			}
		}
	}
