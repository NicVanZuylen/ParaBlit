...
namespace AssetPipeline
{
	struct Bounds
	{
		...
		inline bool RoughlyContains(const glm::vec3& point) const
		{
		...
		inline void Encapsulate(const glm::vec3& point)
		{
			m_extents.x = glm::max(MaxX(), point.x);
			m_extents.y = glm::max(MaxY(), point.y);
			m_extents.z = glm::max(MaxZ(), point.z);

			m_origin.x = glm::min(m_origin.x, point.x);
			m_origin.y = glm::min(m_origin.y, point.y);
			m_origin.z = glm::min(m_origin.z, point.z);
