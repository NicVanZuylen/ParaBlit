...
void main() 
{
    ...
    for(int i = 0; i < count; ++i)
    {
        ...
        float normalDotLight = max(dot(normal, light.direction.xyz), 0.0);
        float normalDotCam = max(dot(normal, dirToCam), 0.0);

        vec3 lightReflected = reflect(light.direction.xyz, normal);
        float specTerm = max(dot(lightReflected, -dirToCam), 0.0);

        vec3 diffuse = normalDotLight * light.color.rgb;
        vec3 spec = normalDotLight * pow(specTerm, SPECULAR_FOCUS) * SPECULAR_POWER * light.color.rgb;
